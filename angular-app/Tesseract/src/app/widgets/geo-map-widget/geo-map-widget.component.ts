...
export class GeoMapWidgetComponent extends AbstractWidget {
    ...
    Object.keys(result.columns).forEach(colName => {
      ...
      if (!this.customOptions["series" + series.length + "Config"])
        this.customOptions["series" + series.length + "Config"] = {
          name: "Series " + colName + " Config : E.g map([<Gradient Colors>], <min>, <max>) or mapbubble(<Bubble Color>) or mappoint(<marker>)",
          value: "map([#ff8484, #ff0000])",
          group: colName + " Series"
        }
  ...
  private updateSeriesFromCustomOptions(series, id) {
    ...
    if (config.startsWith("mapbubble"))
      this.handleMapBubble(config, series);
    else if (config.startsWith("mappoint"))
      this.handleMapPoint(config, series);
  ...
  private handleMapPoint(config: string, series: any) {
    let markerUrl = config.replace("mappoint", "").replace("(", "").replace(")", "").trim();
    let pointData = series.data.map(e => {
      if (e["value"] != null && e["value"] != 0)
        return {
          x: e["x"] && e["y"] ? e["x"] : e["_midX"],
          y: e["x"] && e["y"] ? e["y"] : e["_midY"],
          visible: true,
          value: e["value"],
          name: e["name"],
          z: e["z"]
    ...
    series.update({
      ...
      data: pointData,
      marker: {
        symbol: markerUrl ? "url(" + markerUrl + ")" : null,
        width: 20,
        height: 20,
        fillColor: 'white',
        lineColor: 'black',
  ...
  private handleMapBubble(config: string, series: any) {
    let color = config.replace("mapbubble", "").replace("(", "").replace(")", "").trim();
    series.update({
      type: "mapbubble",
      color: color
    });
